          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-20">
            ...
            <motion.div style={{ opacity: formOpacity }} className="lg:col-span-6">
              <form onSubmit={handleSubmit} onMouseEnter={() => setIsHoveringCard(true)} onMouseLeave={() => setIsHoveringCard(false)} className="space-y-4 bg-black/40 p-5 md:p-8 rounded-2xl border border-[#A855C5]/20 shadow-[0_30px_60px_rgba(0,0,0,0.7)] relative overflow-hidden group">
                ...
                {/* Custom "Not a Robot" Verification */}
                <div className="relative z-10 flex items-center gap-3 p-4 bg-white/[0.03] border border-white/5 rounded-lg group/captcha cursor-pointer select-none transition-all duration-300 hover:bg-white/[0.05]" onClick={() => setIsVerified(!isVerified)}>
                  <div className={`w-5 h-5 rounded border-2 transition-all duration-300 flex items-center justify-center ${isVerified ? 'bg-[#A855C5] border-[#A855C5] shadow-[0_0_10px_rgba(168,85,197,0.5)]' : 'border-white/20 group-hover/captcha:border-[#c79a40]/50'}`}>
                    {isVerified && (
                      <motion.svg 
                  ...
                  <span className={`text-[10px] tracking-[0.2em] uppercase font-bold transition-colors duration-300 ${isVerified ? 'text-white' : 'text-white/40 group-hover/captcha:text-white/60'}`}>
                    I am not a robot
                  </span>
                  <div className="ml-auto opacity-20 group-hover/captcha:opacity-40 transition-opacity">
